...
const { convertToAwsXRayTraceId, generateXAmznTraceId } = require('../shared/trace-utils');
const { extractCommonFields, cleanGcpFields } = require('../shared/field-cleaner');
...
const formatAwsLog = (object) => {
    ...
    if (fields.context.traceId) {
        ...
        result['x-amzn-trace-id'] = generateXAmznTraceId(fields.context.traceId, fields.context.spanId);
    }
    ...
    if (requestMethod || requestUrl || clientIp || contentLength || userAgent) {
        ...
        if (contentLength) result.request.contentLength = parseInt(contentLength) || contentLength;
